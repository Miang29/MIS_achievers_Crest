...
<div class="container-fluid m-0">
	<h2 class="my-3"><a href="{{ route('service.index', [$id]) }}" class="text-decoration-none  text-1"><i class="fas fa-chevron-left mr-2"></i>Services</a></h2>
	<hr class="hr-thick" style="border-color: #707070;">

	<div class="row" id="form-area">
		<div class="col-12">

			<form class="card my-3 mx-auto"action="{{ route('submit-service-variation',[$id]) }}" method="POST" enctype="multipart/form-data">
				<h3 class="card-header  text-white gbg-1"><i class="fa-solid fa-square-plus mr-2"></i>CREATE SERVICE</h3>

				<div class="card-body"  class="form">
					{{ csrf_field() }}
					...
					<div class="row">
						<div class="col-12 col-md-12">
							<div class="input-group mb-3 col-lg-12">
								...
								<select class="custom-select" id="inputGroupSelect01" name="service_name">
									@foreach ($services as $s)
									<option value="{{ $s->id}}">{{ $s->service_name }}</option>
									@endforeach
								</select>
								<small class="text-danger small">{{$errors->first('service_name')}}</small>
							</div>
						</div>
				...
				<div class="card-footer d-flex">
					<div class="col-4 mx-auto text-center">
						<button type="submit" class="btn btn-outline-info btn-sm w-50" data-type="submit" data-action="submit">Save</button>
						<a href="javascript:void(0);" onclick="confirmLeave('{{ route('service.index',[$id]) }}');" class="btn btn-outline-danger btn-sm w-50">Cancel</a>
					</div>
				</div>
			</form>
		</div>
