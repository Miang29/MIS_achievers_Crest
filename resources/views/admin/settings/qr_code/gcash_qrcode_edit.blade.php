...
@section('title', 'Edit Gcash Information')
...
<div class="container-fluid m-0">
	...
	<form class="card mx-auto" method="POST" action="{{ route('submit.boarding') }}" enctype="multipart/form-data">
		{{ csrf_field() }}
		<h3 class="font-weight-bold text-center mt-5 text-primary">Edit Gcash Information</h3>
		<div class="card col-lg-10 col-md-12 col-12 my-2 mx-auto mb-5">
			...
			<div class="col-12 col-md-12 col-lg-6 mx-auto mt-3 border border-secondary bg-primary">
				{{-- IMAGE INPUT --}}
				<div class="image-input-scope" id="gcash-image-scope" data-settings="#image-input-settings" data-fallback-img="{{ asset('uploads/settings/default.png') }}">
					{{-- FILE IMAGE --}}
					<div class="form-group text-center image-input collapse show avatar_holder" id="gcash-image-input-wrapper">
						<div class="row py-2 mx-1">
							<div class="col-12 col-md-6 text-md-right">
								<div class="hover-cam mx-auto avatar rounded overflow-hidden">
									<img src="{{ asset('uploads/settings/default.png') }}" class="hover-zoom img-fluid avatar" id="gcash-image-container" alt="Gcash Image" data-default-src="{{ asset('uploads/settings/default.png') }}">
									<span class="icon text-center image-input-float" id="gcash-image" tabindex="0" data-target="#gcash-image-container">
										<i class="fas fa-camera text-white hover-icon-2x"></i>
									</span>
								</div>
								<input type="file" name="gcash_image" class="d-none" accept=".jpg,.jpeg,.png,.webp" data-role="image-input" data-target-image-container="#gcash-image-container" data-target-name-container="#gcash-image-name">
							</div>

							<div class="col-12 col-md-6 text-md-left">
								<label class="form-label font-weight-bold text-white mt-2" for="gcash-image">Gcash QR Code</label><br>
...
<script type="text/javascript">
	$(document).ready(() => {
		$(document).on('click', ".reset-image", (e) => {
			$(e.currentTarget).closest(".image-input").find(`#gcash-image-container_${$(e.currentTarget).attr('data-reset-id')}`).attr('src', `{{ asset('uploads/settings/default.png') }}`);
		});
		instantiateSelect();
	});
</script>
